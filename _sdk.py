...
def SetWheelVal(val):
    SetAxis(val, 1, HID_USAGE_X)

def SetRightBlinker(val):
	SetBtn(val, 1, 1)

def SetLeftBlinker(val):
	SetBtn(val, 1, 2)
...
class _JOYSTICK_POSITION_V2(Structure):
    ...
    AcquireVJD(1)
    ResetVJD(1)
    # SetAxis(0, 1, HID_USAGE_X)
    SetAxis(1000000, 1, HID_USAGE_X)
    SetBtn(True, 1, 1)
    SetBtn(True, 1, 2)
    SetBtn(True, 1, 3)
    # SetAxis(1, 1, HID_USAGE_SL0)
    # SetAxis(0, 1, HID_USAGE_SL1)
    # SetAxis(0, 1, HID_USAGE_WHL)

    # SetAxis(0, 1, HID_USAGE_Z)
    # SetAxis(0, 1, HID_USAGE_RX)
    # SetAxis(0, 1, HID_USAGE_RY)
